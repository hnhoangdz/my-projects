...
from preprocessor import _resize as resize
from preprocessor import to_categorical
import scipy.ndimage as ndi
import cv2

# READ: https://viblo.asia/p/tang-cuong-du-lieu-trong-deep-learning-oOVlYe4nl8W
class ImageGenerator(object):
    def __init__(self, ground_truth_data,batch_size,image_size,
                 train_keys,validation_keys,ground_truth_transformer=None,
                 path_prefix=None,
                 saturation_var=.5,
                 brightness_var=.5,
                 contrast_var=.5,
        ...
        if saturation_var:
            self.saturation_var = saturation_var # Độ bão hòa
            self.color_jitter.append(self.saturation)
        if brightness_var:
            self.brightness_var = brightness_var # Đô sáng
            self.color_jitter.append(self.brightness)
        if contrast_var:
            self.contrast_var = contrast_var # Độ tương phản
            self.color_jitter.append(self.contrast)
        self.lighting_std = lighting_std # Độ sáng tb
        self.horizontal_flip_probability = horizontal_flip_probability # Quay ngang
        self.vertical_flip_probability = vertical_flip_probability # Quay dọc
        self.do_random_crop = do_random_crop # Cẳt
        self.zoom_range = zoom_range # Zoom
        self.translation_factor = translation_factor
    
    # Random crop ảnh
    def _do_random_crop(self, img_array):
        ...
        crop_matrix = np.array([[scale_factor, 0],
                                [0, scale_factor]])
        img_array = np.rollaxis(img_array, axis=-1, start=0)
        image_channel = [ndi.interpolation.affine_transform(image_channel,
                         crop_matrix, offset=offset, order=0, mode='nearest',
                         cval=0.0) for image_channel in img_array]

        img_array = np.stack(image_channel, axis=0)
        img_array = np.rollaxis(img_array, 0, 3)
        return img_array
    
    # Random xoay ảnh
    def do_random_rotation(self, img_array):
        height = img_array.shape[0]
        width = img_array.shape[1]
        x_offset = np.random.uniform(0, self.translation_factor * width)
        y_offset = np.random.uniform(0, self.translation_factor * height)
    ...
    # 
    def _gray_scale(self, img_array):
        return img_array.dot([0.299, 0.587, 0.114])
    
    # Chuan hoa ve do bao hoa
    def saturation(self, img_array):
        gray_scale = self._gray_scale(img_array)
        alpha = 2.0*np.random.random()*self.brightness_var
        alpha = alpha + 1 - self.saturation_var
        img_array = (alpha * img_array + (1 - alpha) *
                       gray_scale[:, :, None])
        return np.clip(img_array, 0, 255)
    
    # Chuan hoa ve do sang
